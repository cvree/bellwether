...
> **2026-09-14 addendum — 1.39.0: the standing record.**
> The first collection in this app that is not a diary. Everything until now was one row per day/meal/dose ordered by when it happened; a diagnosis, an allergy, a bereavement and a third-floor flat are none of those. One new module, one new screen, one db slice, one profile field.
> - **`src/lib/record.ts` — pure, and the whole feature.** `HealthFact` is **one shape with sixteen `kind`s across three `FactDomain`s** (`body` 7 / `mind` 3 / `life` 6), per-kind optional fields, `KIND_META` (label, one, blurb, add, `noneLine`, `ask`), `STATUSES_FOR`, `COMMON` one-tap catalogs, `IMPACT_KINDS`. Sixteen collections would be sixteen sanitisers, sixteen sync kinds and sixteen backup keys to express one idea — the `kind` field does it, exactly as `LabResult.kind` keeps an estimate off a measurement's axis.
> - **One list, not three, is the load-bearing taxonomy decision.** A depression diagnosis is a `condition`, a psychiatric admission a `procedure`, an antidepressant a `RoutineItem`. Quarantining mental facts into their own collection is a clinical error before it is a discourtesy, and it is the thing a biopsychosocial history exists to refuse.
> - **`RecordState` is a separate object from the facts, and that is the point.** It records what has been *asked* — `none[]`, `statedAt` per kind, `reviewedAt`. "No known drug allergies, stated 14 March" is a clinical statement; "nobody asked" is not; an app printing an empty heading for both has told the reader something false about one. The pack prints facts / a dated sentence / **named as not asked**, and `buildRecordTable` keeps the same three-way split into the spreadsheet.
> - **`sanitizeRecordState(v, facts)` takes the facts as an argument** and drops any `none` for a kind that holds something — a file claiming both is repaired with the typed fact winning. `migrateDb` therefore sanitises `d.record` *before* `d.profile.record`; reversing those two lines reintroduces the contradiction.
> - **Partial dates.** `YYYY` / `YYYY-MM` / `YYYY-MM-DD`, kept at the precision given and never padded — `dateFloor` exists for ordering only. `datePrecision` runs a real calendar check (`dayExists`): the shape regex alone accepts `2019-02-30`, and a date that cannot have happened is a typo, not a lower-precision date. `yearsSince` floors.
> - **`private` + `shareable()` is the single outbound gate.** Pack, XLSX and JSON export all run through it; backup and sync deliberately do not (a backup is the journal; the other device is the same person's). `packRecordSection` computes **`missing` from the shareable set too**, so a kind holding only private facts reads as *not asked* on paper. `newFact` starts `event` private (`PRIVATE_BY_DEFAULT`) — a default, visible and one tap either way, not a rule.
> - **`alertFacts` never infers severity from a reaction string.** "throat closes" is a description; promoting it to anaphylaxis would be the app making a clinical judgement. Only a severity the person set, plus `pinned`, reaches the bordered box — which prints black, not coloured, because packs go out in mono.
> - **One renderer** (`factLine` / `factDetail` / `whenLabel` / `factText`) used by the screen, the pack, the export and the search index. Four copies would be four chances for the pack and the screen to describe the same fact differently, which is how somebody stops trusting a document.
> - **`src/components/RecordScreen.tsx`.** Three domain cards → sixteen sections → editor sheet, plus `RunThrough` (one kind per screen, multi-select chips, Add / Nothing to add / Skip / Stop). No ring, no percentage, no tick: a person with nothing to say about their family history has a complete record, not an 81% one.
> - **Wiring:** `db.record` + `profile.record`, both in `migrateDb`, `buildFullBackup`, `restoreBackup`; `"fact"` `RecordKind` + `["fact", "record"]` in sync `COLLECTIONS`; `screen === "record"`; a History door (first in the row), a Settings card, `PLACES` entry, `SearchKind` `"fact"`, and a new `person` icon in `Icon`. `PACK_SECTIONS` gains `record` **first** and `DEFAULT_PACK_SECTIONS.record` is true, so existing installs get it on.
> - **Gotchas.**
>   1. **`newFact` must not run through `sanitizeFact`.** A draft legitimately has an empty label; the sanitiser correctly rejects one; the fallback label meant every fresh editor opened with "Condition" already typed in. The sanitiser belongs at the storage boundary, not in the constructor.
>   2. **A status a kind does not offer is dropped, never translated.** Mapping a "former" condition to "resolved" would be inventing a clinical statement out of a malformed field.
>   3. **Search gives a fact no `date`.** A `since` of "2019" is not a day; in the date field it would land inside every range filter crossing a new year.
>   4. **`clearNone` clears `statedAt` in the same write.** A date with no statement attached is a fact about nothing.
>   5. **The design-system suite will catch new CSS.** `--fhj-muted` colours nothing readable, `999px` must be `--fhj-radius-pill`, and any duration in 90–460ms must be a named rung (`--fhj-quick` etc.). All three bit on the first run of this feature's stylesheet.
> - **Tests: 95 new across 2 new suites and 4 existing** — `record` (46), `recordUi` (27), `appointmentPack` (+7), `backupCompat` (+3), `syncMerge` (+5), `search` (+7). The assertions that matter are negative: no private fact reaches a pack, an export or an alert band, and no unasked kind is ever printed as a negative. **Totals: 2,250 across 84 suites** (was 2,155/82).
> - **Known, pre-existing, untouched:** `APP_VERSION` in App.tsx still reads `"1.29.0"` while package.json is at 1.39.0. It is printed on the appointment pack footer and the export header. Not changed here because it is unrelated drift, but it is wrong on a document a clinician reads.
